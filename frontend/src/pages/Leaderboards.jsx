...
import { Award, Zap, ShieldAlert, Sparkles, Heart, Eye, TrendingUp, Shield } from 'lucide-react';
...
export default function Leaderboards() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-10 font-sans">
      ...
      {/* 2. Top-10 Category Sub-Tables divided by Batting and Bowling/Fielding */}
      
      {/* Batting Leaders Section */}
      <div className="space-y-4 pt-2">
        <div className="flex items-center gap-3 border-b border-slate-200 dark:border-dark-border pb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-3">
              {bestAverage.map((p, idx) => (
                <div key={p._id} className="flex items-center justify-between text-sm py-1 border-b border-slate-50 dark:border-dark-border/20 last:border-0">
                  <div className="flex items-center gap-2">
                    <span className="w-5 font-bold text-slate-400">{idx + 1}</span>
                    <Link to={`/players/${p._id}`} className="font-bold text-slate-850 dark:text-slate-200 hover:underline">{p.name}</Link>
                  </div>
                  <span className="font-mono text-slate-900 dark:text-white font-bold">{p.battingAverage}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Bowling & Fielding Leaders Section */}
      <div className="space-y-4 pt-2">
        <div className="flex items-center gap-3 border-b border-slate-200 dark:border-dark-border pb-2">
          <div className="h-6 w-1 bg-purple-600 rounded-full"></div>
          <h2 className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-wider">
            Bowling & Fielding Leaders
          </h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-2xl p-6 shadow-sm">
            <h3 className="text-base font-bold mb-4 border-b border-slate-100 dark:border-dark-border pb-3 flex items-center gap-1.5">
              ...
              Most Dot Balls (Top 10)
            </h3>
            <div className="space-y-3">
              {mostDotBalls.map((p, idx) => (
                <div key={p._id} className="flex items-center justify-between text-sm py-1 border-b border-slate-50 dark:border-dark-border/20 last:border-0">
                  <div className="flex items-center gap-2">
                    <span className="w-5 font-bold text-slate-400">{idx + 1}</span>
                    <Link to={`/players/${p._id}`} className="font-bold text-slate-850 dark:text-slate-200 hover:underline">{p.name}</Link>
                  </div>
                  <span className="font-black text-slate-900 dark:text-white">{p.stats.bowling.dotBalls} dots</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-3">
              {mostCatches.map((p, idx) => (
                <div key={p._id} className="flex items-center justify-between text-sm py-1 border-b border-slate-50 dark:border-dark-border/20 last:border-0">
                  <div className="flex items-center gap-2">
                    <span className="w-5 font-bold text-slate-400">{idx + 1}</span>
                    <Link to={`/players/${p._id}`} className="font-bold text-slate-850 dark:text-slate-200 hover:underline">{p.name}</Link>
                  </div>
                  <span className="font-black text-slate-900 dark:text-white">{p.stats.fielding.catches} catches</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Emerging Players */}
          <div className="bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-2xl p-6 shadow-sm col-span-1 md:col-span-2">
            <h3 className="text-base font-bold mb-4 border-b border-slate-100 dark:border-dark-border pb-3 flex items-center gap-1.5">
      ...
      {/* Hall of Shame */}
      <div className="bg-red-950/5 dark:bg-red-950/30 border border-red-200/70 dark:border-red-900/60 rounded-[32px] p-6 shadow-[0_24px_80px_-40px_rgba(220,38,38,0.55)]">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-base font-bold text-red-950 dark:text-red-100">Hall of Shame</h3>
            <p className="text-sm text-red-700/80 dark:text-red-200/70">Top 3 worst offenders for ducks, low averages, and expensive bowling.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="rounded-3xl border border-red-200/80 bg-white dark:bg-red-950/80 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h4 className="text-xs font-bold uppercase tracking-[0.24em] text-red-600">Worst Average</h4>
                <p className="text-[11px] text-red-700 dark:text-red-300 mt-1">Min 2 innings, top 3 only.</p>
              </div>
              <span className="rounded-full bg-red-100 px-2 py-1 text-[11px] font-semibold text-red-800 dark:bg-red-800/70 dark:text-red-100">Low Average</span>
            </div>
            <div className="space-y-3">
              {worstAverage.length === 0 ? (
                <p className="text-sm text-slate-500">No average data available yet.</p>
              ) : (
                worstAverage.slice(0, 3).map((p, idx) => (
                  <div key={p._id} className="flex items-center gap-3 rounded-2xl border border-red-100 bg-red-50/80 p-3 text-sm text-red-950 dark:border-red-700/50 dark:bg-red-900/50 dark:text-red-100">
                    <span className="min-w-[24px] text-lg font-black text-red-650">{idx + 1}</span>
                    <div className="min-w-0 grow">
                      <Link to={`/players/${p._id}`} className="block truncate font-bold hover:underline">{p.name || p.player?.name || 'Unknown Player'}</Link>
                      <p className="text-[11px] text-red-700/80 dark:text-red-200/70">Average: <span className="font-semibold">{p.battingAverage ?? '-'}</span></p>
                    </div>
                    <span className="rounded-full bg-red-600 px-2 py-1 text-[11px] font-semibold text-white">{p.battingAverage ?? '-'}</span>
                  </div>
                ))
              )}
            </div>
